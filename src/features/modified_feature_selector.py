...
############################################################################################################
import modified_feature_selection
############################################################################################################
...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    """
    # added multiclass and n_significant attributes
    def __init__(self, test_for_binary_target_binary_feature=defaults.TEST_FOR_BINARY_TARGET_BINARY_FEATURE,
                 ...
                 multiclass=False, n_significant=1):
        """
 ...
 ############################################################################################################
    ...
    def fit(self, X, y):
        ...
        if not isinstance(y, pd.Series):
            y = pd.Series(y.copy())
############################################################################################################
        # added multiclass and n_significant arguments
        relevance_table = calculate_relevance_table(
            X, y, ml_task=self.ml_task,
            multiclass=self.multiclass, n_significant=self.n_significant,
            n_jobs=self.n_jobs,
...
###########################################################################################################
        if self.multiclass:
            # in the multiclass case, the p_values attribute is now a DataFrame that has the p_values for each class
            self.feature_importances_ = None
            self.p_values = relevance_table.filter(regex='^p_value_*', axis=1)
            # 1-p.value**n_significant
            # importance: 
            # argument switches between avg/max/min/all p-values
            
            # :param multiclass_p_values: 'avg','max','min','all' , default = 'min'
            # :type multiclass_p_values: str
            
            # in the case that multiclass_p_values='all'
            # feature_importances_ is a dataframe
